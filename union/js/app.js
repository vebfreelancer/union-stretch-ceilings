...
let burger = document.querySelector('.actions-header__burger');
let catalog = document.querySelector('.catalog');
let catalogMenu = document.getElementById('catalog_desctop');
let menuClose = document.querySelector('.menu-close');
let catalogMenuBodu = document.querySelector('.catalog-menu__body');

let menuHeight = menuBody.clientHeight - catalogMenuBodu.clientHeight;
let menuFullHeight = menuBody.clientHeight;
let catalogHeight = catalog.clientHeight;

function actionsHeader(){
	let delay = 500;
	header.style.width = document.querySelector('.wrapper').clientWidth + 'px';

	if (window.innerHeight < menuBody.clientHeight){
	...
	if (unlock){
		if (catalog.classList.contains('_active') && window.innerWidth <= 1100){
			body_lock(delay);
			headerTop.classList.remove("_active");
			menuBody.classList.remove("_active");
			catalog.classList.remove("_active");
		}
		if (window.innerWidth > 1100 && menumobile.classList.contains('_active')){
			body_lock(delay);
			menumobile.classList.remove("_active");
		}
		// if (window.innerHeight < catalogHeight && catalog.classList.contains('_active')){
		// 	catalog.classList.add('_scroll');
		// }
		// setTimeout(removeScrollCatalog, 1000);

		// if (!catalog.classList.contains('_scroll')){
		// 	catalog.style.width = document.querySelector('.wrapper').clientWidth + 'px';
		// } else catalog.removeAttribute('style');
	}
}
actionsHeader();

// function removeScrollCatalog(){
// 	if (window.innerHeight > catalogHeight || !catalog.classList.contains('_active')){
// 		catalog.classList.remove('_scroll');
// 	}
// }

if (burger != null || catalogMenu != null) {
	let delay = 500;
	window.addEventListener('click', (event) => {
		const target = event.target;
		
		if (unlock) {
			if (target.closest('.actions-header__burger')){
				body_lock(delay);
			...
			if (target.closest('#catalog_desctop')){
				body_lock(delay);
				if (window.innerWidth > 1100){
					catalog.classList.toggle("_active");
				}
				if (catalogMenu.classList.contains('_active') && window.innerWidth <= 1101){
					menuBody.classList.add('scroll');
				} else {
					menuBody.classList.remove('scroll');
				}
		...
		if (target.closest('#catalog_desctop') && window.innerWidth > 1100){
...
function menu_close() {
	let iconMenu = document.querySelector(".actions-header__burger");
	let menuBody = document.querySelector(".bottom-header");
...
function _load(){
	catalog.classList.add('_load');
	menumobile.classList.add('_load');
	popups.forEach(el => {
